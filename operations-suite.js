(() => {
  ...
  function renderChat() {
    ...
    if (!container) return;
    container.innerHTML = '<article class="card form-card chat-compose"><div class="section-heading"><div><h2>Nachricht schreiben</h2><p class="muted">Einfach schreiben und senden. Alle berechtigten Nutzer sehen denselben Verlauf.</p></div><span class="ops-chip">Dauerhaft gespeichert</span></div><label>Nachricht<textarea id="chat-text" placeholder="Nachricht schreiben …"></textarea></label><div class="form-actions"><button class="primary" data-ops-action="send-chat">Senden</button></div><p class="muted">Enter sendet · Umschalt + Enter fügt eine neue Zeile ein. Nachrichten bleiben erhalten, solange der gemeinsame Datenbestand nicht administrativ zurückgesetzt wird.</p></article><section class="section"><div class="section-heading"><div><h2>Chatverlauf</h2><p class="muted">' + ops.chatMessages.length + ' dauerhaft gespeicherte Nachrichten</p></div><input id="chat-search" type="search" value="' + esc(chatSearch) + '" placeholder="Verlauf durchsuchen" aria-label="Chatverlauf durchsuchen" /></div><div id="chat-stream" class="chat-stream">' + stream + '</div></section>';
