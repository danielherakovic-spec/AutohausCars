// Browser integration test with a mocked Supabase module. No live data is read or changed.
...
const mockModule = `export function createClient(){
 ...
 channel(){return chain},removeChannel(){},storage:{from(){return {createSignedUrl:async()=>({data:{signedUrl:'https://test.invalid/car.png'}})}}},
 rpc:async()=>({data:state})}; }
`;
...
(async () => {
  ...
  try {
    ...
    console.log('PASS: empty state, notes, colors, dragging, threads, duplicates, comparison, persistence, deletion, undo, keyboard, export/import, mobile, no JS errors.');
    await context.close();
